<template xmlns:c="http://www.w3.org/1999/html">
    <div>
        <van-form>
            <van-cell title="故障类型">{{columnslx|optionMapKV(info.gzlx)}}</van-cell>
            <van-cell title="故障地点">{{info.gzdd}}</van-cell>
    ...
    export default {
        ...
        methods: {
            ...
            selectinfo(){
                ...
                _this.$ajax.post(process.env.VUE_APP_SERVER + '/wxbase/reportFacilityfail/selectinfo',
                    formData
                ).then((response)=>{
