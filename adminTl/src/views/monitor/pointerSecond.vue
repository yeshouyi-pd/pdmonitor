...
<!--    <div id="echartsBox" style="display: flex;flex-wrap: wrap;"></div>-->
    <div class="widget-box">
      <div class="widget-header">
        <h4 class="widget-title">实时水下环境指针预警数据</h4>
      </div>
          ...
          <form>
            <table style="font-size: 1.1em;width:80%" class="text-right">
              ...
              <tr>
                ...
                <td style="width: 20%">
                  <select v-model="pointerSecondDto.sm" class="form-control">
                ...
                <td style="width: 20%">
                  <times v-bind:startTime="startTime" v-bind:endTime="endTime" start-id="secondStartId" end-id="secondEndId"></times>
                </td>
    ...
    <div>
      <table id="simple-table" class="table  table-bordered table-hover">
        ...
        <tbody>
        <tr v-for="pointerSecond in pointerSeconds">
          <td>{{waterEquipments|optionNSArray(pointerSecond.sm)}}</td>
          <td>{{pointerSecond.sm}}</td>
          <td>{{pointerSecond.decibelValue}}</td>
          <td>{{pointerSecond.cjsj}}</td>
        </tr>
        </tbody>
      </table>
...
export default {
  ...
  methods: {
    ...
    startTime(rep){
      let _this = this;
      _this.pointerSecondDto.stime = rep;
      _this.$forceUpdate();
    },
    /**
     *结束时间
    ...
    endTime(rep){
      let _this = this;
      _this.pointerSecondDto.etime = rep;
      _this.$forceUpdate();
    },
    list(page){
      let _this = this;
      ...
      _this.pointerSecondDto.page = page;
      _this.pointerSecondDto.size = _this.$refs.pagination.size;
      _this.$ajax.post(process.env.VUE_APP_SERVER + '/monitor/admin/pointerSecond/list',_this.pointerSecondDto).then((response)=>{
        Loading.hide();
        let resp = response.data;
        _this.pointerSeconds = resp.content.list;
