  ...
  <div class="wrap" id="app">
    <header style="position: relative;">
      ...
      <div style="position: absolute;right: 20px;">
        <img style="width: 250px" src="/static/image/jsbg.png" alt="">
      </div>
    </header>
    <div class="content">
      <div class="pain">
...
export default {
  ...
  methods: {
    ...
    getThreeDayTs(){
      ...
      let obj = {
        'sbbhs':'NJA4002',
        'etime':Tool.dateFormat("yyyy-MM-dd",new Date(new Date().getTime()-3600000*24*1)),
        'stime':Tool.dateFormat("yyyy-MM-dd",new Date(new Date().getTime()-3600000*24*7))
      }
      _this.$ajax.post(process.env.VUE_APP_SERVER + '/monitor/welcome/getThreeDayTsPost',obj).then((res)=>{
        let response = res.data;
    ...
    getA4AndA2JL(){
      ...
      let obj = {
        'sbbhs':'NJA4002',
        'etime':Tool.dateFormat("yyyy-MM-dd",new Date()),
        'stime':Tool.dateFormat("yyyy-MM-dd",new Date(new Date().getTime()-3600000*24*6))
      }
      _this.$ajax.post(process.env.VUE_APP_SERVER + '/monitor/welcome/getEventDataPost',obj).then((res)=>{
        let response = res.data;
    ...
    getRightTopData(){
      ...
      obj.sbbh="NJA4002";
      obj.topDayType="3";
      _this.$ajax.post(process.env.VUE_APP_SERVER + '/monitor/welcome/statisticsAlarmNumsByTimeSum', obj).then((response) => {
        let resp = response.data;
        let alarmDatas = resp.content;
        let zccs = { label: '本周声学侦测次数', value: alarmDatas.num };
        let sjcs = { label: '本周事件(群次)', value: alarmDatas.nnm };
        let bscs = { label: '本周捕食次数', value: alarmDatas.bnum };
