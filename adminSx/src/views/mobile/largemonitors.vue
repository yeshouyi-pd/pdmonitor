<template>
  <div class="wrap" id="app">
    <header style="position: relative;">
      ...
      <img src="/largemonitors/assets/imgs/rigthpic.png" style="position: absolute; right: 2%; top: 5px; width: 180px;">
    </header>
    <div class="content">
      <div class="pain">
        <div class="h37">
...
<!--              </div>-->
<!--            </div>-->
            <div style="display: flex;justify-content: space-around;align-items: center;clear: both;margin-top: 15px;">
...
export default {
  ...
  methods: {
    ...
    getThreeDayTs(){
      let _this = this;
      _this.$ajax.post(process.env.VUE_APP_SERVER + '/monitor/welcome/getThreeDayTsPost',{"sbbhs":"SXA4001"}).then((res)=>{
        let response = res.data;
    ...
    getA4AndA2JL(){
      let _this = this;
      _this.$ajax.post(process.env.VUE_APP_SERVER + '/monitor/welcome/getEventDataPost',{'deptcode':Tool.getLoginUser().deptcode}).then((res)=>{
        let response = res.data;
    ...
    getRightTopData(){
      ...
      _this.$ajax.post(process.env.VUE_APP_SERVER + "/monitor/welcome/statisticsAlarmNumsByTimeSum", obj).then((response) => {
        ...
        let zccs = { label: '声学侦测次数', value: alarmDatas.num==null?0:alarmDatas.num };
        let sjcs = { label: '事件(群次)', value: alarmDatas.nnm==null?0:alarmDatas.nnm };
        let bscs = { label: '捕食次数', value: alarmDatas.bnum==null?0:alarmDatas.bnum };
