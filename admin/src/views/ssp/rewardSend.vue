                  ...
                  <tr  >
                    ...
                    <td style="width: 8%">
                      <select v-model="webchatRewardDto.cplx" class="form-control">
                        ...
                        <option v-for="(value, key) in allcplx" v-bind:value="value">{{value}}</option>
                      </select>
                    </td>
                    <td style="width:6%" >
                      车牌号码：
      ...
      <tr v-for="webchatReward in webchatRewards">
        ...
        <td>
          <div class="hidden-sm hidden-xs btn-group">
            <button v-on:click="edit(webchatReward)" class="btn btn-xs btn-info" title="发放奖励">
              <i class="ace-icon fa fa-pencil bigger-120"></i>
            </button>
<!--            <button v-on:click="del(webchatReward.id)" class="btn btn-xs btn-danger">-->
...
<!--              </div>-->
          ...
          <div class="modal-footer">
            <button v-on:click="wxPay()" type="button" class="btn btn-primary">发放奖励</button>
...
<script>
  ...
  export default {
    ...
    mounted: function() {
      ...
      // sidebar激活样式方法一
      // this.$parent.activeSidebar("wxbase-rewardSend-sidebar");
    ...
    methods: {decryptByDES(ciphertext) {
      ...
      list(page) {
        ...
        _this.webchatRewardDto.ffzt="0";
        _this.$forceUpdate();
        Loading.show();
        _this.$ajax.post(process.env.VUE_APP_SERVER + '/wxbase/admin/webchatReward/list',_this.webchatRewardDto).then((response)=>{
          Loading.hide();
          let resp = response.data;
      ...
      save() {
        ...
        // 保存校验
        if (1 != 1
                || !Validator.length(_this.rewardSend.createBy, "创建人", 1, 30)
                || !Validator.length(_this.rewardSend.updateBy, "更新人", 1, 30)
                || !Validator.length(_this.rewardSend.sysOrgCode, "所属部门", 1, 50)
                || !Validator.length(_this.rewardSend.wfxwxx, "违法行为详细", 1, 200)
                || !Validator.length(_this.rewardSend.sfzmhm, "身份证明号码", 1, 20)
                || !Validator.length(_this.rewardSend.sjh, "手机号", 1, 11)
                || !Validator.length(_this.rewardSend.shr, "审核人", 1, 30)
                || !Validator.length(_this.rewardSend.shyy, "审核原因", 1, 30)
                || !Validator.length(_this.rewardSend.shsm, "审核说明", 1, 500)
                || !Validator.length(_this.rewardSend.zt, "状态", 1, 1)
                || !Validator.length(_this.rewardSend.openid, "用户openid", 1, 40)
                || !Validator.length(_this.rewardSend.lsh, "流水号", 1, 40)
                || !Validator.length(_this.rewardSend.ffzt, "奖励发放状态", 1, 1)
                || !Validator.length(_this.rewardSend.cplx, "车牌类型", 1, 20)
                || !Validator.length(_this.rewardSend.cphm, "车牌号码", 1, 20)
                || !Validator.length(_this.rewardSend.ffr, "发放人", 1, 20)
        ) {
          return;
        }

        Loading.show();
        _this.$ajax.post(process.env.VUE_APP_SERVER + '/wxbase/admin/rewardSend/save', _this.rewardSend).then((response)=>{
          Loading.hide();
          let resp = response.data;
          if (resp.success) {
      ...
      del(id) {
        ...
        Confirm.show("删除后不可恢复，确认删除？", function () {
          Loading.show();
          _this.$ajax.delete(process.env.VUE_APP_SERVER + '/wxbase/admin/rewardSend/delete/' + id).then((response)=>{
            Loading.hide();
            let resp = response.data;
            if (resp.success) {
              _this.list(1);
      ...
      wxPay() {
        ...
        Loading.show();
        _this.$ajax.post(process.env.VUE_APP_SERVER + '/wxbase/admin/webchatReward/wXPay', _this.webchatReward).then((response)=>{
          Loading.hide();
          let resp = response.data;
          if (resp.success) {
            $("#form-modal").modal("hide");
