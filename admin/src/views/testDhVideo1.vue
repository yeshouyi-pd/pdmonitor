<template>
  <div class="row" style=" height:600px;width: auto;background-color: #1c84c6">
...
export default {
  data() {
    return {
      ...
      stats: '',
      error: '',
      playUrl:"http://192.168.10.124:19091/monitor/realPlayManager/startPlay/0",
      websocketUrl:'ws://192.168.10.124:19091/monitor/device/monitor/0',
    };
  },
  mounted() {
    //this.loadFlv();
  ...
  methods: {
    ...
    async getLogin(){
      const url2 = this.playUrl; // 设置POST请求的URL
      const data = {};
      try {
    ...
    setupWebSocket() {
      ...
      let ws = new WebSocket('ws://'+this.serverIp +':'+this.wsPort+'/monitor/device/monitor/0');
    ...
    loadFlv() {
      ...
      this.player = this.flv.init(
          {
            type: 'flv',
            url: 'ws://'+this.serverIp +':'+this.wsPort+'/monitor/device/monitor/0',
            isLive: true,
