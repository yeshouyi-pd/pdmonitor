...
export default {
  ...
  methods: {
    ...
    list(page) {
      ...
      _this.queryuser.xmbm="002";
      _this.$forceUpdate();
      _this.$ajax.post(process.env.VUE_APP_SERVER + '/system/admin/userVideo/list', _this.queryuser).then((response)=>{
        Loading.hide();
        let resp = response.data;
    ...
    save() {
      ...
      _this.user.rode = "00000001";
      Loading.show();
      _this.$ajax.post(process.env.VUE_APP_SERVER + '/system/admin/userVideo/save', _this.user).then((response)=>{
        Loading.hide();
        let resp = response.data;
        if (resp.success) {
    ...
    del(id) {
      ...
      Confirm.show("删除用户后不可恢复，确认删除？", function () {
        Loading.show();
        _this.$ajax.delete(process.env.VUE_APP_SERVER + '/system/admin/userVideo/delete/' + id).then((response)=>{
          Loading.hide();
          let resp = response.data;
          if (resp.success) {
            _this.list(1);
