 ...
 */
@Component
public class FDownloadDataCallBack implements NetSDKLib.fDataCallBack{
    @Autowired
    ...
    public int invoke(NetSDKLib.LLong lRealHandle, int dwDataType, Pointer pBuffer, int dwBufSize, Pointer dwUser) {
        System.out.println("视频回放"+lRealHandle);
        ...
        if (info.getEmDataType() == 0 || info.getEmDataType() == 3) {
            ...
            if (dwDataType == 0) {
//                sendBuffer(byteArray, lRealHandle.longValue());
            }
    ...
    private void sendBuffer(byte[] bytes, long realPlayHandler) {
        ...
        server.sendReportMessageToOne(realPlayHandler, buffer);
    }
}
